...
*In this tutorial, we will... (donner un mini aperçu de ce qui sera couvert.) On pourrait dire par exemple qu'on va utiliser l'optimal foraging theory pour prédire le comportement optimal durant TrophIE?*
...
Optimal diet and optimal residency time can be combined in a hierarchical model of optimal foraging in patchy landscapes [@mcnamaraCombiningPreyChoice1993]. Prey then have to decide which prey type(s) should be consumed in each patch, and when they should leave the current patch. In risky environments, their decisions should also consider their risk of mortality when involved in different behaviour. Optimal decisions thus depend on multiple factors, including the risk of predation, the level of safety provided by refuges, the time required to handle each resource, and the gain of handling a given resource.

What are the optimal multi-level foraging decisions under these conditions? We estimate this optimality problem numerically through a two-step process. First, we estimate the gain that can be expected within patches from the different possible foraging decisions. Each set of foraging decisions within the patches yields the total gain (g) during the residence time (t). Second, we use this information [g(t), t] as input to the inter-patch model to estimate the overall realized gain that can be expected during Tf = 30 min, given the global mortality risk. Although a PowerPoint presentation details the modeling approach, below we summarize the general method through an example. 
...
We  now estimate the first step of the gain function g(0), starting from the animal's arrival in the patch. We consider a typical type II functional response [@fryxellIndividualBehaviorCommunity1997], where the encounter rate with each type of marble is the product of the "number of marbles that can be drawn per minute (lambda_in)" and the "proportion of that type of marble available in the bag" upon arrival in the patch (i.e., `N1_in_t0 / N_in_t0`). 
...
h1_in <- (10 / 60)     # handling time prey type 1 (min / marble)
h2_in <- (10 / 60)     # handling time prey type 1 (min/marble)

e1_in <- 60            # points gained by consuming a prey of type 1 (points/marble)
e2_in <- 10            # points gained by consuming a prey of type 1 (points/marble)
p1_in <- 1             # if marble of type 1 are taken, p = 1; 0 otherwise
p2_in <- 0             # if marble of type 2 are taken, p = 1; 0 otherwise
```
...
Now that we have determined the expect gain in a patch, given the choice of marble type and the time spent in each patch, we can use this information to estimate the expected overall gain after the allowed 30 minutes.
...
N1 <- 100 / 112000 # density of patch type 1  (patches / m2) (i.e., 100 patches in about 120 000 m2)
...
Because  you specifically have 30 min to collect marbles, we consider the behaviour you will be doing when the time is up. For example, you may not be able to spent the time you wanted in the last patch. The approach below considers this possibility when calculating the overall gain that can be expected in 30 minutes. The model provides estimates of the optimal time spent in refuges, searching for marble patches, and collecting marbles in patches, given your foraging choice and the risk of predation.  

As you will see, the model does not directly identify the optimal diet. To determine the most rewarding choice, all you have to do is estimate the long-term expected payouts if you collect only one type of marble or if you collect both. You will then be able to directly see which strategy offers the highest expected gains. The model provides an estimate the time that should be spent in refuges, at marble patches, and moving between patches, to maximize your gains, given the long-term mortality risk. 
...
c <- cbind(Variable1, Value_given_optimal_behaviour)
...
The code generates a figure showing the gain rate ($ / time spent in patch) that can be expected in a patch, as a function of the time spent in that patch. The code also generates two columns providing eight attributes of the simulation, including the behaviours yielding the highest payoffs, given the diet choice. 
...
ggplot(data = dat, aes(x = x, y = y)) +
    ...
    scale_y_continuous(breaks = seq(0, 30, 6), limits = c(6, 30)) +
    theme_bw(base_size = 15) +
    theme(panel.grid = element_blank())
```
