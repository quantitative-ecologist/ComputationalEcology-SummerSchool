...
All analyses in this tutorial were done using the R software, but note that they could also work in Python or Julia.
...
We now build the multivariate mixed-effects models for three predator foraging behaviours being the predator's average speed, rate of space covered, and latency before the first capture. As you will see, the two models follow a very similar structure.

**In the first model**, we are interested in modelling the predator foraging behaviour without any effects of the prey. The goal is to describe the predator's foraging tactics in its most "basic" form.

**In the second model**, we build upon the first model, but we include the prey's behaviour as fixed effects. This will help us assess whether the predator's foraging tactics described in the first model change after removing the effect of the prey's average speed and rate of space covered.

If we see a difference in the predator's foraging tactics between the two models, then we can infer that the prey are driving the predator's foraging tactics. 
...
The variance-covariance matrices (i.e. equations 4 to 6) is the part of the model enabling us to determine the structure of the predator's foraging tactics. For instance, at the individual level, if we see that the predator's speed and latency before the first capture are negatively correlated, then we can infer that faster predators are more efficient because they take less time to capture their first prey. On the other hand, the residual correlations tell a similar story, but at the within-individual level. Thus, using the same correlation as an example, we could say that matches in which predators were faster were also matches in which they were more efficient at capturing the prey. Alternatively, if individuals were slower in other matches, then they were not as efficient.

In the next subsections, we will write our models using the `brms` syntax. As you will see, we will use the `bf()` function (for "**b**rms **f**ormula") in `brms` to write the model formulas. We will employ these naming conventions for the equations describing the predator's traits (i.e. equation 1 to 3) :
...
For the predator's speed and rate of space covered, the model estimates a general intercept (1.e. the `1` term) and two random effects which are the `avatar_id` and the `predator_id`. The model for the predator's latency before the first capture follows the same structure, but we include the match duration (`Zgame_duration`) as a fixed effect to control for differences in game length.

The `a` and `b` indicate that we want the model to estimate correlations among the random effects. Thus, the `avatar_id` and `predator_id` random effects will be correlated across the three traits. Note that in `brms`, you could have used any letter to estimate the correlations, as long as you keep the same letter across sub models.
...
Lastly, the Bayesian approach requires that we define priors for the parameters that the models will estimate. In short, priors can be defined as a probability distribution indicating the prior plausibilities of an uncertain quantity before evidence is taken into account. They are usually assumed and carefully chosen based on the scientific knowledge of the system to help the model at estimating parameters. Thus, they can be seen as an assumption of how your system of interest works.
...
You can see that the range of plausible values for the slope ranges from ~-2 to ~4 with the most plausible value being 0.5 (i.e. the black dashed line).

In our case, because this was the first time that we modelled data in *Dead by Daylight*, we use weakly informative priors which impose constrains on the parameter estimation while also leaving it some freedom. Moreover, because we have a large dataset, the data should drive the posterior distribution of parameter values more than the prior. For more information on priors, you can read the marvellous book of [Richard McElreath called Statistical Rethinking](https://xcelab.net/rm/statistical-rethinking/). 
...
Estimating model parameters with MCMC can take a long time. Moreover, the more data you have, the longer the computation will be. For example, in a single script, both models together took around 1.5 hours to run on a supercomputer with 64Gb of RAM and 48 computer cores.

Thus, whenever your models take some time to run, you should save their results (e.g. as `.rds` files or any other file depending on your programming language) when they have finished running. You can then reupload the model outputs as R objects into an R session and manipulate them for your needs (e.g. for checks, plots, and tables).
...
And here is the `brms` code used to fit the second model, where we controlled for the prey's behaviour (`fit2`).
...
We first inspect the convergence of the chains for every parameter of interest using trace plots. We can deduce that the chains converged well when the trace plots are homogeneous. For simplicity, we will only inspect the chains of the first model, but remember that we should do the same steps for the second model too.
...
In `brms`, the values always appear beside the parameter means displayed using the `summary()` function. For simplicity, we display them in a `data.table`. We will inspect the values for the fixed effects, followed by those of the random effects + correlations.
...
Here, $y$ is the distribution of our data, and $y_{rep}$ is the distribution simulated data from the model's posterior predictive distribution. If the model is a good fit, we should se a strong overlap between the curves, which is mostly the case here except for the predator's speed. For speed, we see that our observed data (i.e. %y%) is a bit off on the left compared to the simulated data from our model. We could thus in theory adjust the model in some way to increase it's predictive power.
...
Now that we confirmed that our two models can be trusted, we will first evaluate their fixed effects. We use the `summary()` function to display them along with their credible intervals. In `brms`, the fixed effects are called population-level parameters.
...
# References and suggested readings {.unnumbered}
