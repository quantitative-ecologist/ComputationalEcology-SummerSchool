...
Here is an example. Suppose that you want to estimate the relationship between species richness and forest cover using a generalized linear model. Because we usually know that more habitat should support more species, we could use a prior on the slope term defined as a Gaussian distribution with mean of 0.5 and standard deviation of 1. This prior would be telling our model that the slope (relationship between species richness and land cover) should be positive with a range of plausible values oscillating around 0.5, indicating a 0.5 unit increase in species richness for each unit of forest cover. This would be called a weakly informative prior, because the standard deviation of 1 still allows the model to search for a fair range of slope values around the mean of 0.5.
...
You can see that the distribution of plausible values for the slope ranges from -2.31 to 3.74. Values nearing 0.5 (i.e. the black dashed line) have a higher density, and thus, are more likely to be observed.
...
### Predator ID : Differences in behaviour among individual predators

As we mentionned before, we controlled for the predator player's avatar, which is a feature of the system since it is a videogame. Because we are not interested in anything biological about this effect, we won't present its results here, and go directly to describe the results for the predator ID (`predator_id`), that is, **among individual differences in behaviour**. 

In `brms`, the random effects are displayed as standard deviations (`sd`). Thus, the random effects in our models are standard deviations estimated for each predator trait. The standard deviation values indicate whether predator players differ in their predicted average behaviours. For example, the value of `0.557` for the predator's speed indicates that there are moderate differences among individuals in their average speed. Thus, some players are slower while others are faster on average.

Here is a table showing the standard deviations of model 1 and model 2:
...
ranefs_a <- rbind(
  ranefs1[Parameter %like% "sd"],
  ranefs2[Parameter %like% "sd"]
)

# Display the standard deviations table
ranefs_a
```

We can see that the standard deviation for speed does not really change. There is however a decrease in standard deviation for the predator's rate of space coverage when controlling for prey behaviour, and a smaller decrease in the standard deviation for the latency before the first capture. This indicates that among individual differences stay the same for the predator's speed, but differences among individuals decrease when we control for the prey's movement.

Note that we are presently comparing the values qualitatively. You could however compare them quantitatively using contrasts. The procedure would be to extract the posterior distributions of the six standard deviation values, compute the differences between each simulated value, and then report the posterior mean difference along with a range of credible intervals for each combination of standard deviations being compared.

### Predator ID : Correlations between predator behaviours

The correlations (`cor`) between the predator players' average behaviours indicates whether, for example, individuals that were faster also captured their first prey faster (e.g. `cor(Zpredspeed_Intercept,Zlatency1stcapture_Intercept)`). We visualize the correlation values with their credible intervals :

```{r}

# Correlations table
ranefs_b <- rbind(
  ranefs1[Parameter %like% "cor"],
  ranefs2[Parameter %like% "cor"]
)

ranefs_b
```

A better way to visualize and compare them is using a correlation matrix.

```{r, cor-mat, fig.height = 8, fig.width = 15}
...
# Table model1 for the correlation matrix
cormat1 <- ranefs_b[Model == "model1", .(var1, var2, Estimate)]
rows1 <- data.table(
  var1 = c("speed", "space", "latency"),
  var2 = c("speed", "space", "latency"),
  Estimate = c(1, 1, 1)
)
cormat1 <- rbind(cormat1, rows1)

# Table model 2 for the correlation matrix 
cormat2 <- ranefs_b[Model == "model2", .(var1, var2, Estimate)]
rows2 <- data.table(
  var1 = c("speed", "space", "latency"),
  var2 = c("speed", "space", "latency"),
  Estimate = c(1, 1, 1)
)
cormat2 <- rbind(cormat2, rows2)



# Plot the data ----------------------------------------------------------------

# One global custom theme
custom_theme <- theme(
    panel.grid = element_blank(),
...
cormat_p1 <- ggplot(data = cormat1, aes(x = var1, y = var2, fill = Estimate)) +
  geom_tile(
    color = "white",
    lwd = 1.5,
    linetype = 1
...
cormat_p2 <- ggplot(data = cormat2, aes(x = var1, y = var2, fill = Estimate)) +
  geom_tile(
    color = "white",
    lwd = 1.5,
    linetype = 1
...
We see that there are structural changes in the correlations, with reversed signs when comparing the correlations of speed and space between model 1 and 2. There is also a reversing of the correlation direction for space coverage and performance between model 1 and model 2.


## Additional analyses : variance partitionning

see @FraserFranco.etal2022 for the results on variance partitionning.
...
Our results suggest that different ecological processes are happening. First, we saw that controlling for the prey's behaviour changes the mean population behaviours. We also saw that controlling for the prey's behaviour changes the estimation of among individual behavioural differences. Importantly, we also see that there are important structural changes in the behavioural correlations.

Overall, we can see that predators differ in their average speed. This means that some individuals were on average faster, while others were on average slower across the study. We also found among individual differences in performance, indicating that some individuals are faster at capturing their first prey than others. This means that we have the basic material for selection to act upon!

When we don't account for the prey's behaviour, we see that faster predators do not cover space faster in the environment. There is actually almost no relationship between these two traits. In terms of performance, faster predators take less time to capture their first prey, indicating higher foraging performance. On the contrary, individuals that cover space in the environment at a faster pace perform poorly compared to those that cover space at a slower rate. Thus, being faster appears to be the best strategy while not covering a lot of space.

However, when we control for the prey's behaviour (i.e. remove the effect of the prey), this interpretation changes drastically. Now, the predator's speed and space coverage are strongly positively correlated, indicating that faster predators also cover space faster in the virtual environment. We also now see that it is organized into a tactic, where faster predators that cover space faster perform better. We thus have strong evidence that the prey are driving the predator's foraging tactics. This indicates that because of the prey's movement, predators cannot be fast and cover a lot of space in the environment at the same time. If we did not account for the prey's behaviour, we would have not arrived to this conclusion, showing that the two interacting parties should be simultaneously analyzed if we want to draw proper conclusions.
