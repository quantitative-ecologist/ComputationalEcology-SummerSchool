...
In `brms`, the values always appear beside the parameter means displayed using the `summary()` function. For simplicity, we display them in a `data.table`. Here are the values for the fixed effects, followed by those of the random effects + correlations :
...
All the values seem to fall below the tresholds, which is good.
